...
class bzr_hr_expense(osv.osv):
    ...
    _columns = {
        'name': fields.char(u'摘要', size=200, required=True),
        'type_id': fields.many2one('bzr.hr.expense.type', u'报销类型', required=True),
        ...
        'date': fields.date(u'报销日期', required=True),
        'line_ids': fields.one2many('bzr.hr.expense.line', 'order_id', u'报销明细'),
...
class bzr_hr_expense_line(osv.osv):
    ...
    _columns = {
        ...
        'order_id': fields.many2one('bzr.hr.expense', u'报销单', required=True, ondelete='cascade'),
